...
class ProsaveOrderData(models.Model):
    ...
    def cron_process_prosave_order(self):
        ...
        for record in records:
                try:
                    ...
                    self._log_error(_("Error processing record: %s") % str(e))

        # Re-trigger cron if more records exist
        if len(records) == 100:
    ...
    def process_prosave_order(self):
        ...
        try:
            ...
            self._log_error(_("Invalid JSON data: %s") % str(e))
            return
        ...
        self._create_purchase_order(data)
    ...
    def _validate_data(self, data):
        ...
        for field, method in [
            ('UserEmail', self._get_user_id),
            ('Supplier', self._get_supplier_id),
            ('Project', self._get_analytic_account_id)
            ]:
    ...
    def _create_purchase_order(self, data):
        order_lines = self._prepare_order_lines(data.get('Details', []), self.env.user.company_id.pro_save_po_default_product_id.id)
        purchase_order = self.env['purchase.order'].create({
            'partner_id': self._get_supplier_id(data['Supplier'])[0],
            'user_id': self._get_user_id(data['UserEmail'])[0],
            'name': data['OrderNumber'],
            'analytic_distribution_id': self._get_analytic_account_id(data['Project'])[0],
    ...
    def _validate_order_data(self, data):
        required_fields = ['UserEmail', 'CVR',
                           'OrderNumber', 'Supplier', 'Project']
        missing_fields = [
            field for field in required_fields if field not in data]
        return missing_fields

    def _get_user_id_org(self, email):
